...
export default class BitcoinWallet implements IWallet {
  ...
  constructor(
    ...
    this.addressDbUtil = addressDbUtil;
    const hash = crypto
      .createHash('sha256')
      .update(xpub)
      .digest('hex')
  ...
  public async getChainAddressIndex(
    ...
    if (this.addressDbUtil) {
      const cacheResult = await this.addressDbUtil(
        'getChainIndex',
        address,
        this.xpub,
        this.coinType
  ...
  public async generateUnsignedTransaction(
    ...
    try {
      ...
      const addressFromDB = await this.addressDbUtil('getAll', {
        xpub: this.xpub,
        coinType: this.coinType
      });
  ...
  private async getInitialAddressListFromServer() {
    ...
    if (xResp.data.tokens && xResp.data.tokens.length > 0) {
      for (const token of xResp.data.tokens) {
        ...
        await this.addressDbUtil('insert', {
          address,
          xpub: this.xpub,
          coinType: this.coinType,
          chainIndex,
